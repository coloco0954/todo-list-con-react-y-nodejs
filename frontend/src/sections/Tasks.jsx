...
const Tasks = ({ tasks, searchValue, onClickCheck, onClickDelete, onClickUpdate, onChangeTitle, onChangeDescription, onChangePriority, title, description, priority }) => {
    ...
    return (
        <div className="flex items-center justify-center">
            {tasks.length === 0 ? 'No hay tareas agegadas' : <div>
                {tasks
                    .filter(task => {
                    ...
                    .map(task => {
                        return (
                            ...
                            <div key={task.id} id={task.id} className={task.completed ? 'mb-4 line-through' : 'mb-4'}>
                                <div className="flex flex-row bg-blue-chill-400 w-[900px] px-2 py-2 justify-between">
                                    <div className="flex flex-row gap-x-3">
                                        <CheckTask onClick={onClickCheck} isCompleted={task.completed} />
                                        <h1 className="text-2xl font-medium">{task.title}</h1>
                                        <p className="mt-1 ml-3">prioridad (<strong className={task.priority}>{task.priority}</strong>)</p>
                                    </div>
                                ...
                                <div className="bg-blue-chill-300 h-auto px-2 py-3 hidden" id="description-container">
